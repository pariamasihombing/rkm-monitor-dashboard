...
export default function PICAmbahProgram() {
  ...
  const [activeMenu, setActiveMenu] = useState(
    fromRoute === "/pic-non-rkm" ? "Non RKM" : "RKM / Program"
  );
  ...
  const handleSimpan = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert("Gagal menyimpan program: " + JSON.stringify(errorData));
      }
    } catch (error) {
  ...
  const handleBatal = () => navigate(fromRoute);
          ...
          >
            <CardContent sx={{ p: 4, pb: "32px !important" }}>
              ...
              <Typography fontSize="0.88rem" sx={{ color: "#727989", mb: 3 }}>
                Isi detail program yang ingin ditambah ke dalam sistem monitoring.
              </Typography>
              ...
              {/* ── Unified grid: row1 = Tipe | Initiative(span2), row2 = Status | PlanStart | PlanEnd ── */}
              <Box sx={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 1.5fr", gap: 2, mb: 2.5 }}>

                {/* Tipe Program — col 1 */}
                <Box>
                  <Typography fontSize="0.85rem" fontWeight={600} sx={{ color: "#1E293B", mb: 0.6 }}>
                    Tipe Program<span style={{ color: "#E9004A" }}>*</span>
                  </Typography>
                  <TextField
                    select
                    fullWidth
                  ...
                  >
                    <MenuItem value="" disabled>Pilih Tipe Program...</MenuItem>
                    {TIPE_PROGRAM_OPTIONS.map((opt) => (
                      <MenuItem key={opt} value={opt}>{opt}</MenuItem>
                    ))}
                  </TextField>
                </Box>

                {/* Initiative Strategy — col 2 + 3 */}
                <Box sx={{ gridColumn: "span 2" }}>
                  ...
                  <TextField
                    ...
                    onChange={(e) => setInitiativeStrategy(e.target.value)}
                    sx={fieldSx}
                    SelectProps={{ displayEmpty: true }}
                  >
                    <MenuItem value="" disabled>Pilih Initiative Strategy...</MenuItem>
                    {INITIATIVE_STRATEGY_OPTIONS.map((opt) => (
                      <MenuItem key={opt} value={opt}>{opt}</MenuItem>
                    ))}
                  </TextField>
                </Box>
              ...
              {/* ── PIC (grouped: utama, supporting, supervisor) ── */}
              <Box sx={{ mb: 3.5 }}>
                <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 1.2 }}>
                  <Typography fontSize="0.85rem" fontWeight={600} sx={{ color: "#1E293B" }}>
                    PIC
                  </Typography>
                  {totalSelected > 0 && (
                    <Typography
                ...
                <Box sx={{ border: "1px solid #E2E8F0", borderRadius: "10px", p: 2, bgcolor: "#FAFBFC" }}>
                  ...
                  {/* PIC UTAMA */}
                  <Typography fontSize="0.72rem" fontWeight={700} sx={{ color: "#64748B", letterSpacing: "0.5px", mb: 1, textTransform: "uppercase" }}>
                    PIC Utama
                  </Typography>
                  <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1.5, mb: 1.5 }}>
                    {PIC_UTAMA_LIST.map((name) =>
                      renderPill(name, selectedUtama.includes(name), () => toggleGroup(name, selectedUtama, setSelectedUtama), selectedSupporting.includes(name))
                    )}
                  </Box>
                  ...
                  <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1.5, mb: 1.5 }}>
                    {PIC_SUPPORTING_LIST.map((name) =>
                      renderPill(name, selectedSupporting.includes(name), () => toggleGroup(name, selectedSupporting, setSelectedSupporting), selectedUtama.includes(name))
                    )}
                  </Box>
                  ...
                  <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1.5 }}>
                    {PIC_SUPERVISOR_LIST.map((name) =>
                      renderPill(name, selectedSupervisor.includes(name), () => toggleGroup(name, selectedSupervisor, setSelectedSupervisor))
                    )}
                  </Box>
                </Box>
                ...
                {totalSelected > 0 && (
                  <Box sx={{ mt: 1.2, display: "flex", alignItems: "center", gap: 0.8 }}>
                    <Typography fontSize="0.78rem" sx={{ color: "#64748B" }}>
                      Dipilih:
                    </Typography>
                    <Typography fontSize="0.78rem" fontWeight={600} sx={{ color: "#0C4B7D" }}>
                      {[...selectedUtama, ...selectedSupporting, ...selectedSupervisor].join(", ")}
                    </Typography>
                  </Box>
                )}
              </Box>
