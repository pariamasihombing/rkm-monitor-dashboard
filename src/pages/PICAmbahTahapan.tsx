...
export default function PICAmbahTahapan() {
  ...
  const handleSimpan = async () => {
    if (!programId) {
      console.error("No programId found");
      alert("Gagal menyimpan: ID Program tidak ditemukan.");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        navigate(returnTo || "/pic-program-detail", { state: { from: fromRoute, programId } });
      } else {
        const errorData = await response.json();
        console.error("Failed to save stage:", errorData);
        
        let errorMessage = "Gagal menyimpan tahapan. Silakan periksa input Anda.";
      ...
      alert("Terjadi kesalahan koneksi saat menyimpan tahapan.");
    }
  };

  const handleBatal = () => navigate(returnTo || "/pic-program-detail", { state: { from: fromRoute, programId } });
          ...
          >
            <CardContent sx={{ p: 4, pb: "32px !important" }}>
              ...
              <Typography fontSize="0.88rem" sx={{ color: "#727989", mb: 3 }}>
                Tahapan berisi kumpulan subtask. Progress tahapan dihitung otomatis oleh sistem.
              </Typography>
