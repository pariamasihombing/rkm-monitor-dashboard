...
export default function PICProgramDetail() {
      ...
      if (typeof programId === "string" && (programId.startsWith("rkm-") || programId.startsWith("nonrkm-"))) {
        ...
        if (normalized.sections && normalized.sections.length > 0) {
          setOpenSections((prev) => {
            // Keep existing open states if any
            const next = { ...prev };
            if (Object.keys(next).length === 0) {
               next[normalized.sections[0].id] = true;
            }
            return next;
          });
        }
          ...
          >
            <CardContent sx={{ p: 3, pb: "20px !important" }}>
              ...
              <Box sx={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", mb: 1.5 }}>
                ...
                <Button
                  size="small"
                  onClick={() => navigate("/pic-edit-program", { 
                    state: { 
                      from: fromRoute, 
                      returnTo: "/pic-program-detail", 
                      programId,
                      namaProgram: program.title || program.name,
                      tipeProgram: program.type || "RKM",
                      initiativeStrategy: program.initiativeStrategy || program.code_initiative_strategy,
                      status: program.status?.name || "On Progress",
                      planStart: program.plan_start || program.start,
                      planFinish: program.plan_finish || program.deadline,
                      pic: program.pic || [program.picUtama, program.picSupporting, program.picSupervisor].join(" | ")
                    } 
                  })}
                  sx={{
                    textTransform: "none",
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 0.6, mb: 2 }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                  <Typography fontSize="0.82rem" color="#64748B">Initiative Strategy :</Typography>
                  <Typography fontSize="0.82rem" color="#0F172A" fontWeight={500}>{program.initiativeStrategy}</Typography>
                </Box>
                {/* PIC breakdown by role */}
                <Box sx={{ display: "flex", alignItems: "flex-start", gap: 0.5 }}>
                  <Typography fontSize="0.82rem" color="#64748B" sx={{ flexShrink: 0 }}>PIC Utama :</Typography>
                  <Typography fontSize="0.82rem" color="#0F172A" fontWeight={600}> {program.picUtama || program.pic || "-"}</Typography>
                </Box>
                <Box sx={{ display: "flex", alignItems: "flex-start", gap: 0.5 }}>
                  <Typography fontSize="0.82rem" color="#64748B" sx={{ flexShrink: 0 }}>Supporting :</Typography>
                  <Typography fontSize="0.82rem" color="#0F172A" fontWeight={500}> {program.picSupporting || "-"}</Typography>
                </Box>
                <Box sx={{ display: "flex", alignItems: "flex-start", gap: 0.5 }}>
                  <Typography fontSize="0.82rem" color="#64748B" sx={{ flexShrink: 0 }}>Supervisor (VP) :</Typography>
                  <Typography fontSize="0.82rem" color="#0F172A" fontWeight={500}> {program.picSupervisor || "-"}</Typography>
                </Box>
              </Box>
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                <Typography fontSize="0.82rem" color="#0F172A" fontWeight={600}>{program.start}</Typography>
                <ArrowForwardIcon sx={{ fontSize: "0.9rem", color: "#94A3B8" }} />
                <Typography fontSize="0.82rem" color="#64748B">Deadline :</Typography>
                <Typography fontSize="0.82rem" color="#0F172A" fontWeight={600}>{program.deadline}</Typography>
              </Box>
              ...
              <Box sx={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 1.5 }}>
                {[
                  { label: "Actual",   value: `${program.actual}%`,   color: "#2563EB" },
                  { label: "Expected", value: `${program.expected}%`, color: "#0F172A" },
          ...
          >
            <CardContent sx={{ p: 3 }}>
              ...
              <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2.5 }}>
                ...
                <Button
                  size="small"
                  onClick={() => navigate("/pic-tambah-tahapan", { state: { from: fromRoute, returnTo: "/pic-program-detail", programId } })}
                  sx={{
                    textTransform: "none",
                    fontWeight: 700,
                    color: "#2196F3",
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {openSections[section.id] ? (
                          <ArrowDownIcon sx={{ fontSize: "1.1rem", color: "#64748B" }} />
                        ) : (
                          <ArrowRightIcon sx={{ fontSize: "1.1rem", color: "#64748B" }} />
                        )}
                        <Typography fontSize="0.9rem" fontWeight={600} color="#0F172A">
                          {section.title}
                        </Typography>
                        {/* Progress indicator */}
                      ...
                      <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                        <Typography fontSize="0.82rem" fontWeight={600} color="#2563EB">
                        ...
                        <ActionBtn 
                          label="View" 
                          color="#2196F3" 
                          onClick={() => navigate("/pic-tahapan-detail", {
                            state: {
                              from: fromRoute,
                              returnTo: "/pic-program-detail",
                              programId,
                        ...
                        />
                        <Pipe />
                        <ActionBtn label="Edit"  color="#F97316" onClick={() => navigate("/pic-edit-tahapan", {
                            state: {
                              from: fromRoute,
                              returnTo: "/pic-program-detail",
                              programId,
                              sectionId: section.id,
                              namaTahapan: section.title,
                              status: section.status?.name || "On Progress",
                              planStart: section.start || section.plan_start,
                              planFinish: section.deadline || section.plan_finish,
                    ...
                    <Collapse in={openSections[section.id]}>
                      <Box sx={{ p: 1.5, display: "flex", flexDirection: "column", gap: 1 }}>
                                        ...
                                        {task.file && (
                                          ...
                                          >
                                            ...
                                            <Box component="span" sx={{ fontSize: "0.8rem", color: "#2563EB", cursor: "pointer", textDecoration: "underline", textDecorationColor: "rgba(37,99,235,0.4)", textUnderlineOffset: "2px", fontWeight: 500 }}>
                                              {task.file.length > 20 ? task.file.substring(task.file.indexOf('_') + 1) : task.file}
                                            </Box>
                                          </Box>
                                        )}
                                      </Box>
                        ...
                        {/* + Tambah Subtask */}
                        <Box
                          onClick={() => navigate("/pic-tambah-subtask", { state: { from: fromRoute, returnTo: "/pic-program-detail", programId, sectionId: section.id } })}
                          sx={{
                            display: "flex",
                            alignItems: "center",
